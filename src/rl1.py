...
class SAC(Node):
    ...
    def x_callback(self,msg):
        ...
        self.action = self.agent.choose_action(T.tensor(msg.data, dtype=T.float))
        command_msg = Float64MultiArray()
        self.action_list = self.action.tolist()

        command_msg.data =self.action_list
...
class Agent():
    def __init__(self, alpha=0.0003, beta=0.0003, input_dims=[45],
        ...
        self.actor = ActorNetwork(alpha, input_dims, n_actions=n_actions,
                    name='actor',max_action=T.tensor([3.0,3.0,3.0,3.0,3.0,3.0,3.0,3.0,3.0,3.0,3.0,3.0]))
...
class ActorNetwork(nn.Module):
    def __init__(self, alpha, input_dims, max_action , fc1_dims=256, 
            fc2_dims=256, n_actions=12, name='actor', chkpt_dir='tmp/sac'):
    ...
    def forward(self, state):
        ...
        sigma = T.clamp(sigma, min=-1, max=1)
